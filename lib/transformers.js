...
module.exports = function (gulp, $, $env) {
    ...
    exports.browserify = function (configuration) {
        ...
        var stream = through()
                ...
                .pipe(debug())
                .pipe(function () {
                    return source(configuration.filename);
                })
                .pipe(function() {
                    return buffer;
                })
        ...
        globby(configuration.src).then(function(entries) {
            settings.entries = entries;

            browserify(settings).bundle().pipe(stream);
        }).catch(function(error) {
            stream.emit('error', error);
        });

        return $later().pipe(stream);
    };
