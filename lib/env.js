    ...
    getAllConfigurations = function (callback) {
        ...
        if(!loadedModules) {
            buildModules(function ($modules) {
                ...
                if(callback)
                    callback($env, configs, $helpers.shell);
            });
        }
        else {
            for (var module in $modules) {
                ...
                updateModules = function (mods) {
                    for (var name in mods) {
                        if ((
                            ...
                            if ($helpers.shell.test('-d', name) && $helpers.shell.test('-f', configFile)) {
...
var $env = $helpers.shell.test('-f', configFiles[0]) ? require(configFiles[0]) : {};
