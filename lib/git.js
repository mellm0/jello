...
module.exports = function(gulp, $, $env) {
    ...
    var exports = {},
        ...
        currentBranch = function(args) {
            return $shell.exec('git rev-parse --abbrev-ref HEAD ' + args + ' 2>/dev/null', {silent: true}).output.trim();
        },
        originForBranch = function(branch) {
            if(!branch)
                branch = currentBranch();

            return $shell.exec('git config branch.' + branch + '.remote', {silent: true}).output.trim();
        },
            ...
            if ($gUtils.env.modulesOnly) {
                ...
                if ($shell.which('composer') && $shell.test('-d', 'composer.json')) {
                    ...
                    $shell.exec(updateCommand, function () {
                        ...
                        $.log('Updating packages after uploading module changes');
