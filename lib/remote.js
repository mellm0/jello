    ...
    var findTarget = function (target, overrides) {
        ...
        sync = function (configuration, callback, backups, noCommands) {
            if (backups && Array.isArray(backups) && configHasOneOfArrayAsProperty(configuration, backups)) {
                ...
                if (options.hasOwnProperty('dest')) {
                    var dest = $helpers.rtrim($helpers.replace_vars(options.dest, vars), '/'),
                        src,
                    ...
                    if (noCommands) {
                        ...
                        if(fullyRemote) {
                            ...
                            execute(['rsync ' + args.join(' ') + ' ' + src + ' ' + dest + append], {dest: dest, 'dry-run': options.hasOwnProperty('dry-run') && options['dry-run']}, function (code, output) {
                                if (callback) {
                                    callback.apply(this, [dest, src, options, code, output]);
                                }
                            }, dest);
        ...
        execute = function (commands, configuration, callback, dest, execSettings) {
            if (commands && commands.length) {
                ...
                var destFolder = configuration.hasOwnProperty('dest') ? configuration.dest : '~';

                if (destFolder.indexOf('~') !== 0) {
