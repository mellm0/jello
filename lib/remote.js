...
module.exports = function(gulp, $, $env) {
    ...
    var findTarget = function(target) {
            ...
            for (var option in options) {
                if (options.hasOwnProperty(option)) {
                    ...
                    if (options[option] === true) {
                        ...
                        if(options.hasOwnProperty(options[option + '-additional'])) {
        ...
        sync = function(configuration, callback, backups, noCommands) {
            if(backups && Array.isArray(backups) && configHasOneOfArrayAsProperty(configuration, backups)) {
                ...
                for (option in defaults.options) {
                    if (defaults.options.hasOwnProperty(option)) {
                        options[option] = defaults.options[option];
                    }
                }
                ...
                if (options.hasOwnProperty('dest')) {
                    var dest = $helpers.rtrim(replaceVars(options.dest, vars), '/'),
                        src = options.hasOwnProperty('src') ? $helpers.rtrim(replaceVars(options.src, vars), '/') + '/' : pwd + '/';
                    ...
                    if(noCommands) {
                        ...
                        before(dest, src, options, function () {
                            $shell.exec('rsync ' + args.join(' ') + ' ' + src + ' ' + dest, function (code, output) {
                                ...
                                if(output && options.hasOwnProperty('itemize-changes') && typeof options['itemize-changes'] !== 'boolean') {
                                    execute(
        ...
        execute = function(commands, configuration, callback, dest) {
